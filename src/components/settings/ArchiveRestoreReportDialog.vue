  ...
  >
    <div v-if="report" class="space-y-4">
      ...
      >
        ...
        <div class="mt-1 break-all font-mono text-sm text-(--text-secondary)">
        ...
        >
          ...
          <div class="mt-3 space-y-2">
            <div
              ...
              :key="name"
              class="rounded-md border border-(--border-color) px-3 py-2 text-sm text-(--text-secondary)"
            >
              {{ name }}
            </div>
            <div
              v-if="report.restoredCollections.length === 0"
        ...
        >
          ...
          <div class="mt-3 space-y-2">
            <div
              ...
              class="rounded-md border border-[color-mix(in_srgb,var(--danger)_30%,transparent)] px-3 py-2 text-sm text-(--text-secondary)"
            >
              <div class="font-medium text-(--text-primary)">
                {{ failure.collectionName }}
              </div>
              <div class="mt-1 text-(--text-muted)">{{ failure.message }}</div>
            </div>
            <div
              v-if="report.failedCollections.length === 0"
              class="text-sm text-(--text-muted)"
            >
      ...
      >
        ...
        <div
          v-if="report.droppedViewReferences.length > 0"
        ...
        >
          ...
          >
            ...
            <div class="mt-1 text-(--text-muted)">
              {{ warning.referenceType }}: {{ warning.referenceValue }}.
              {{ warning.reason }}
            </div>
          </div>
        </div>
        <div v-else class="mt-3 text-sm text-(--text-muted)">
          No dropped view references.
        </div>
      </div>
        ...
        >
          ...
          <div v-if="report.skippedEntities.length > 0" class="mt-3 space-y-2">
            ...
            >
              ...
              <div class="mt-1 text-(--text-muted)">
                {{ warning.scope }} type "{{ warning.type }}" skipped.
                {{ warning.reason }}
              </div>
            </div>
          </div>
          <div v-else class="mt-3 text-sm text-(--text-muted)">
            No skipped fields or views.
          </div>
        </div>
        <div
          class="rounded-lg border border-(--border-color) bg-(--bg-primary) p-4"
        >
          ...
          <div v-if="report.statMismatches.length > 0" class="mt-3 space-y-2">
            ...
            >
              <div class="font-medium text-(--text-primary)">
                {{
                  mismatch.scope === "collection"
                    ? mismatch.collectionName
                    : "Total database"
                }}
              </div>
              <div class="mt-1 text-(--text-muted)">
                {{ mismatch.stat }} expected {{ mismatch.expected }}, got
                {{ mismatch.actual }}.
              </div>
            </div>
          </div>
          <div v-else class="mt-3 text-sm text-(--text-muted)">
            No stat mismatches detected.
          </div>
        </div>
      </div>
    </div>
