<template>
  <div class="flex h-full min-h-0 flex-col">
    <div v-if="selectFields.length === 0" class="flex flex-1 flex-col items-center justify-center px-8 text-center">
      <i class="pi pi-list mb-4 text-4xl text-[var(--text-muted)]"></i>
      <h3 class="text-lg font-semibold text-[var(--text-primary)]">No Select Fields</h3>
      <p class="mt-2 max-w-md text-base text-[var(--text-muted)]">
        Kanban view requires at least one select field in this collection.
      </p>
    </div>

    <div v-else-if="groupingFieldId !== null && !groupingField" class="flex flex-1 flex-col items-center justify-center px-8 text-center">
      <i class="pi pi-exclamation-circle mb-4 text-4xl text-[var(--text-muted)]"></i>
      <h3 class="text-lg font-semibold text-[var(--text-primary)]">Grouping Field Missing</h3>
      <p class="mt-2 max-w-md text-base text-[var(--text-muted)]">
        The select field used for this Kanban view was deleted. Choose a new field in the Fields panel.
      </p>
    </div>

    <div v-else-if="!groupingField" class="flex flex-1 flex-col items-center justify-center px-8 text-center">
      <i class="pi pi-bars mb-4 text-4xl text-[var(--text-muted)]"></i>
      <h3 class="text-lg font-semibold text-[var(--text-primary)]">Choose a Select Field</h3>
      <p class="mt-2 max-w-md text-base text-[var(--text-muted)]">
        Select the field that should stack items into columns.
      </p>
    </div>

    <div v-else ref="boardRef" class="kanban-board flex min-h-0 flex-1 gap-3 overflow-x-auto overflow-y-hidden p-4"
      @wheel="onBoardWheel">
      <CollectionKanbanColumn v-for="column in columns" :key="String(column.key)" :column="column"
        :viewOrderedFields="orderedFields" :isUncategorized="column.key === null" :colorOptions="groupingOptions"
        @add-item="onAddItem" @edit-item="emit('edit-item', $event)" @card-drop="onCardDrop"
...
function onCardDrop(payload: { itemId: number; targetColumnKey: string | null; afterItemId?: number | null }) {
  ...
  if (currentKey === payload.targetColumnKey) {
    const targetColumn = columns.value.find(column => column.key === payload.targetColumnKey)
    if (!targetColumn) {
      return
    }

    const orderedIds = targetColumn.items.map(entry => entry.id).filter(id => id !== item.id)
    let insertIndex = 0
