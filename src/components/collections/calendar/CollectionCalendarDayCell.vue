<template>
  ...
  >
    <div class="mb-1 flex min-w-0 items-center justify-between gap-1">
      ...
      <span v-if="cell.items.length > 0" class="min-w-0 truncate px-1 text-xs text-[var(--text-muted)]">
    ...
    <div ref="listRef" class="relative min-h-0 flex-1 space-y-1 overflow-hidden">
      <button
        ...
        class="pointer-events-none invisible absolute left-1 top-0 w-full truncate rounded-md px-1.5 py-0.5 text-left text-sm text-[var(--text-primary)]"
        tabindex="-1"
      >
        {{ cell.items[0]?.label ?? "" }}
      </button>
      <CollectionCalendarItemChip v-for="entry in visibleItems" :key="entry.item.id" :entry="entry" @edit-item="emit('edit-item', $event)" />
      ...
      <Popover v-if="hiddenItemCount > 0" v-model:open="popoverOpen">
        <PopoverTrigger as-child>
          <button
            ...
            type="button"
            data-calendar-more
            class="w-full truncate rounded-md px-1.5 py-0.5 text-left text-sm text-[var(--accent-primary)] transition-colors hover:bg-[var(--bg-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)]"
          >
            +{{ hiddenItemCount }} more
          </button>
        </PopoverTrigger>
        <PopoverContent class="w-64 rounded-xl border border-[var(--border-color)] bg-[var(--bg-primary)] p-3 shadow-xl">
          <div class="space-y-2">
