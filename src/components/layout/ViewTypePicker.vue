<template>
  <Transition enter-active-class="transition duration-150 ease-out" enter-from-class="opacity-0 translate-y-1"
    ...
    <div v-if="open" :id="id"
      ...
      :class="placementClass" role="menu" @keydown.esc.prevent="emit('close')">
      <div class="divide-y divide-[var(--border-color)]/70">
        <button v-for="viewType in viewTypes" :key="viewType.type" type="button"
          class="group flex h-10 w-full items-center gap-3 px-3 text-base text-[var(--text-secondary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)]"
          :class="viewType.disabled
            ? 'cursor-not-allowed opacity-60'
            : 'hover:bg-[var(--bg-hover)] hover:text-[var(--text-primary)]'" :disabled="viewType.disabled"
          :title="viewType.tooltip" role="menuitem"
          @click="viewType.disabled ? undefined : emit('select', viewType.type)">
          <i class="pi text-base" :class="[viewType.icon, viewType.iconClass]"></i>
          <span class="flex-1 text-left">{{ viewType.label }}</span>
          <i
            class="pi pi-plus text-base text-[var(--accent-primary)] opacity-70 transition-opacity group-hover:opacity-100"></i>
        </button>
      </div>
    </div>
  </Transition>
