<template>
  <div
    ...
    <div class="mb-1 flex min-w-0 items-center justify-between gap-1 sm:mb-2 sm:gap-2">
      <span
        class="inline-flex size-5 shrink-0 items-center justify-center rounded-full text-[11px] font-semibold sm:size-6 sm:text-base"
        :class="cell.isToday ? 'bg-[var(--accent-light)] text-[var(--accent-primary)]' : ''">
        {{ cell.date.getDate() }}
      </span>
      <span v-if="cell.items.length > 0" class="min-w-0 truncate text-[10px] text-[var(--text-muted)] sm:text-[11px]">
        {{ cell.items.length }}
      </span>
    </div>
    ...
    <div class="min-h-0 flex-1 space-y-1 overflow-hidden">
      ...
      <button v-if="hiddenItemCount > 0" type="button"
        class="w-full truncate rounded-md px-1.5 py-0.5 text-left text-[11px] text-[var(--accent-primary)] transition-colors hover:bg-[var(--bg-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] sm:px-2 sm:py-1 sm:text-base"
        @click="openPopover">
