...
// ---------------------------------------------------------------------------
// buildItemFormDataFromItem – number coercion and fallback branches
// ---------------------------------------------------------------------------
...
describe("buildItemFormDataFromItem", () => {
  ...
  it("leaves null as-is for number fields", () => {
    const fields = [makeField({ id: 1, name: "Score", type: "number" })];
    const item = makeItem({ Score: null });
    const data = buildItemFormDataFromItem(item, fields);
    expect(data.Score).toBe("");
  });
...
// ---------------------------------------------------------------------------
// buildItemDataFromForm – date serialisation and null/undefined values
// ---------------------------------------------------------------------------

describe("buildItemDataFromForm", () => {
  it("serialises a Date object to ISO date string for date fields", () => {
    const fields = [makeField({ id: 1, name: "Due", type: "date" })];
    const formData = { Due: new Date(2025, 5, 15) };
    const result = buildItemDataFromForm(formData, fields);
