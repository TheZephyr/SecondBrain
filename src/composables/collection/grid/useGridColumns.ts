...
export function useGridColumns({
  ...
  const gridTemplateColumns = computed(() => {
    ...
    if (fieldSegments.length === 0) {
      return `${ROW_NUMBER_COLUMN_WIDTH}px ${ADD_FIELD_COLUMN_WIDTH}px 1fr`;
    }

    return `${ROW_NUMBER_COLUMN_WIDTH}px ${fieldSegments.join(" ")} ${ADD_FIELD_COLUMN_WIDTH}px 1fr`;
  });
  ...
  function resetColumnWidth(fieldId: number) {
    if (!Number.isInteger(fieldId) || fieldId <= 0) {
      return;
    }
